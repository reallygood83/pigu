...
const DodgeballSchedule = () => {
  ...
  const renderMatchText = (match) => {
    if (match === '없음') return match;
    
    const teams = match.split(' vs ');
    return (
      React.createElement(React.Fragment, null, 
        React.createElement('span', {style: {color: teamColors[teams[0]], fontWeight: 'bold'}}, teams[0]),
        React.createElement('span', null, ' vs '),
        React.createElement('span', {style: {color: teamColors[teams[1]], fontWeight: 'bold'}}, teams[1])
      )
    );
  };
  ...
  const renderMatchList = (matchList) => {
    return matchList.map((match, index) => {
      ...
      return React.createElement('div', {key: index, className: "mb-1 flex items-center"},
        ...
        React.createElement('div', null, match)
      );
    });
  };

  return React.createElement('div', {className: "bg-gray-100 p-6 font-sans"},
    // Header
    React.createElement('div', {className: "bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 rounded-lg shadow-lg mb-6"},
      React.createElement('div', {className: "flex items-center justify-between"},
        React.createElement('div', null,
          React.createElement('h1', {className: "text-3xl font-bold"}, "박달초 6학년 피구 리그전"),
          React.createElement('p', {className: "text-xl mt-2"}, "박진감 넘치는 대회 일정표")
        ),
    ...
    React.createElement('div', {className: "bg-white rounded-lg shadow-md p-6 mb-6"},
      React.createElement('h2', {className: "text-2xl font-bold mb-4 flex items-center"},
        React.createElement('i', {className: "icon icon-award mr-2 text-yellow-500", 'data-lucide': "award"}),
        "리그 구성"
      ),
      React.createElement('div', {className: "grid grid-cols-1 md:grid-cols-3 gap-4"},
        leagueInfo.map((info, index) => 
          React.createElement('div', {key: index, className: "border rounded-lg p-4 bg-gray-50"},
            React.createElement('h3', {className: "font-bold text-gray-700"}, info.title),
            React.createElement('p', {className: "text-gray-800"}, info.description)
          )
        )
      )
    ),
    ...
    // Team Colors Legend
    React.createElement('div', {className: "bg-white rounded-lg shadow-md p-6 mb-6"},
      React.createElement('h2', {className: "text-xl font-bold mb-4"}, "참가 반 컬러"),
      React.createElement('div', {className: "flex flex-wrap gap-3"},
        Object.entries(teamColors).map(([team, color]) => 
          React.createElement('div', {key: team, className: "flex items-center"},
            React.createElement('div', {className: "w-6 h-6 rounded-full mr-2", style: {backgroundColor: color}}),
            React.createElement('span', {className: "font-bold"}, team)
          )
        )
      )
    ),
    ...
    React.createElement('div', {className: "bg-white rounded-lg shadow-md p-6 mb-6"},
      React.createElement('h2', {className: "text-2xl font-bold mb-4 flex items-center"},
        React.createElement('i', {className: "icon icon-calendar mr-2 text-blue-500", 'data-lucide': "calendar"}),
        ...
        React.createElement('table', {className: "min-w-full border-collapse"},
          React.createElement('thead', null,
            React.createElement('tr', {className: "bg-gray-200"},
              React.createElement('th', {className: "border p-3 text-left"}, "날짜"),
              React.createElement('th', {className: "border p-3 text-left"}, "요일"),
              React.createElement('th', {className: "border p-3 text-left"}, "A코트 경기"),
              React.createElement('th', {className: "border p-3 text-left"}, "B코트 경기"),
              React.createElement('th', {className: "border p-3 text-left"}, "참여 반")
            )
          ),
          React.createElement('tbody', null,
            preliminaryMatches.map((match, index) => 
              React.createElement('tr', {key: index, className: index % 2 === 0 ? 'bg-gray-50' : 'bg-white'},
                React.createElement('td', {className: "border p-3 font-bold"}, match.date),
                React.createElement('td', {className: "border p-3"}, match.day),
                React.createElement('td', {className: "border p-3"}, renderMatchText(match.courtA)),
                React.createElement('td', {className: "border p-3"}, renderMatchText(match.courtB)),
    ...
    React.createElement('div', {className: "bg-white rounded-lg shadow-md p-6 mb-6"},
      React.createElement('h2', {className: "text-2xl font-bold mb-4 flex items-center"},
        React.createElement('i', {className: "icon icon-calendar mr-2 text-red-500", 'data-lucide': "calendar"}),
    ...
    React.createElement('div', {className: "bg-white rounded-lg shadow-md p-6 mb-6"},
      React.createElement('h2', {className: "text-2xl font-bold mb-4 flex items-center"},
        React.createElement('i', {className: "icon icon-clock mr-2 text-green-500", 'data-lucide': "clock"}),
        "반별 경기 일정표"
      ),
      React.createElement('div', {className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"},
        classSchedules.map((schedule) => 
          React.createElement('div', {
            ...
            React.createElement('div', null,
              React.createElement('h4', {className: "font-bold mb-2"}, "사전경기:"),
              renderMatchList(schedule.preliminary),
              React.createElement('h4', {className: "font-bold mt-4 mb-2"}, "본선경기:"),
              renderMatchList(schedule.main)
            )
          )
        )
      )
    ...
    // Footer
    React.createElement('div', {className: "text-center text-gray-600 mt-8"},
      React.createElement('p', null, "박달초등학교 6학년 피구 리그전 2024"),
      React.createElement('p', {className: "mt-2 text-sm"}, "열정과 스포츠맨십으로 가득한 대회를 응원합니다!")
    )
  );
};
...
// 앱 렌더링
ReactDOM.render(
  React.createElement(DodgeballSchedule, null),
